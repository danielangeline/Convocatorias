...
export default async function PortalLayout({ children }: { children: React.ReactNode }) {
  ...
  // Sprint 3 paso 1: y sus proyectos (RN-30).
  const [convocatorias, categorias, proyectos] = await Promise.all([listarCatalogo(), listarCategoriasActivas(), listarProyectos()]);
  ...
  return (
    <div className="flex min-h-screen flex-col bg-white">
      ...
      <footer className="border-t border-line-soft py-6 text-center text-xs text-ink-faint">
        Plataforma de Gestión de Convocatorias — la cuenta, la suscripción, el catálogo y los proyectos son reales; el resto, datos de ejemplo.
      </footer>
      <ModalSuscripcion />
      <ModalCreditos />
    </div>
