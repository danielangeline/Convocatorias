...
begin
  ...
  insert into public.convocatorias (fuente_id, nombre, entidad_convocante, descripcion, ubicacion_cobertura, cobertura_nacional, fecha_cierre, url_postulacion, estado)
    values (v_fuente, 'Conv s016', 'Entidad', 'Objeto', 'Nacional', true, current_date + 30, 'https://entidad.gov.co/x', 'publicada') returning id into v_conv;
  ...
  v_datos := jsonb_build_object('fuente_id', v_fuente, 'nombre', 'Conv s016', 'entidad_convocante', 'Entidad', 'descripcion', 'Objeto',
    'monto_min', '', 'monto_max', '', 'ubicacion_cobertura', 'Nacional', 'cobertura_nacional', true, 'fecha_apertura', '', 'fecha_cierre', (current_date + 30)::text,
  ...
  for v_caso in
    select * from (values
      ...
      ('quitar cobertura',   v_datos || '{"cobertura_nacional":false}', array[v_cat], v_reqs, 'publicada_incompleta'),
