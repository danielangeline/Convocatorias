...
**El paso 3 quedó a medias:** las reglas nuevas de publicación están puestas y probadas en su mayor parte. El cuelgue que impedía verificarlas ya tiene causa: la red de este equipo, no el código (sesión 016). La comprobación completa al guardar ya está restaurada y el último adjunto protegido (sesión 016). Falta **confirmar en otra red** que la suite de publicar pasa entera, para dar el paso por verificado (ver "Bloqueos"). Antes de cargar convocatorias reales conviene corregir **el formato de los montos** (ver hallazgos).
...
| **Un monto escrito con puntos de miles se guarda mal sin avisar.** El campo es `type="number"`: al teclear `1.000.000` el navegador lo deja en `1.000000` y se guarda **1 peso** con "Cambios guardados."; con `abc` se guarda vacío. Reproducido con el teclado en la sesión 016 y comprobado en la base (`monto_min = 1`). Los montos alimentan los filtros del catálogo (RF-12) | `components/admin/EditorConvocatoria.tsx` (montos), `lib/admin/catalogo.ts` (`monto`) | **alta** · corregir antes de cargar convocatorias reales: aceptar el formato colombiano o rechazarlo con mensaje |
| **La segunda barrera rechazó una vez a un administrador con sesión válida:** `POST .../documentos` respondió 404 "No encontrado." en 869 ms, dejó un `acceso_denegado` **sin usuario** y un objeto suelto en el bucket. La petición anterior y las siguientes pasaron. `obtenerSesion()` trata cualquier fallo de `getUser()` como "sin sesión" y no registra el error, así que no se sabe si fue la red o el refresco del token | `lib/auth.ts` (`obtenerSesion`) | media · registrar el error de `getUser()` para diagnosticarlo la próxima vez |
| Los avisos del editor no se limpian: tras adjuntar sigue "falta al menos un documento adjunto", y tras despublicar conviven "Convocatoria despublicada." y el rechazo anterior | `components/admin/EditorConvocatoria.tsx`, sección de documentos | baja · limpiar el aviso de una sección al empezar otra acción |
| La portada muestra "Iniciar sesión" y "Crear cuenta" aunque haya una sesión abierta (con sesión de administrador, `/login` sí redirige al panel) | `app/page.tsx` | baja · ofrecer "Ir a mi portal" cuando hay sesión |
