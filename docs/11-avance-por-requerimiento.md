...
| RF-88 Consentimiento de datos al registrarse *(v6, sesión 021)* | servidor | Casilla obligatoria en el registro de las dos puertas; la acción la vuelve a exigir y la base guarda `perfiles.consentimiento_datos_at`, que el cliente no puede escribir. La cuenta anterior lo acepta en el editor (`aceptar_consentimiento_datos()`) y sin él no guarda (409). Comprobado en SQL y HTTP. Falta verlo en pantalla |
...
| RF-46 Indicador de completitud accionable | verificado | **Sesión 018:** la completitud la calcula un trigger al guardar y no se puede falsear escribiéndola directamente (comprobado); `lib/proyectos.ts` enumera en pantalla los campos que faltan con la misma regla (0, 11, 44 y 100 comprobados). **Sesión 019:** el Product Owner lo revisó en pantalla con su cuenta de empresa (pendiente 10): funciona |
...
| RF-25 Motivo de rechazo y reenvío | verificado | **Sesión 021:** el consultor ve el motivo y reenvía desde `rechazado`; el motivo se conserva para el administrador. El rechazo con motivo desde la bandeja llega en el paso 1b (RF-34). Visto en pantalla por el Product Owner en el pendiente 15 (sesión 022): rechazo con motivo, reenvío y aprobación |
...
| RF-73 Ir al portal como acción primaria | verificado | **Sesión 020:** el detalle de la postulación lee del servidor el enlace oficial (validado al guardar, RNF-29) y lo presenta como botón primario, antes de "Solicitar consultor"; comprobado en el HTML por `prueba-postulaciones.mjs`. Jerarquía visual comprobada en el navegador: botón primario de ancho completo, encima de "Solicitar consultor" |
...
| RF | Estado | Nota |
|---|---|---|
| RF-34 Aprobar/rechazar perfiles | verificado | **Sesión 022:** `revisar_perfil_consultor()` (docs/05 §9.21): solo desde `en_revision`, con la fila bloqueada, revisor y fecha; rechazar exige motivo (RN-13); aprobar no crea suscripción (RN-11 transitorio). Bandeja como componente de servidor con sitio web, redes, portafolio, el motivo del rechazo anterior en un reenvío y la hoja de vida por URL firmada (RNF-16). `revision_consultores.sql` 29/29 y `prueba-revision-consultores.mjs` 51/51, dos corridas. **Recorrida en pantalla por el Product Owner** con su cuenta de Propietario (pendiente 15, sesión 022): en la base, el perfil quedó aprobado por el Propietario |
| RF-35 Suspender y reactivar | verificado | **Sesión 022:** `suspender_consultor()` con motivo obligatorio que ve el consultor, y `reactivar_consultor()`, que no revive encargos (CU-27 3a). El motivo no es legible para la empresa. Mismas pruebas que RF-34. Recorrido en pantalla por el Product Owner (pendiente 15) |
...
| RF-66 Límite de tasa | pendiente | Sprint 5, con Upstash Redis |
...
| RNF-05 Rendimiento de sugerencias | servidor | 3 | **Sesión 019:** con 2 000 convocatorias adicionales la función responde en **141 ms** y el endpoint completo, en 1,6 s por la red. **Sin estadísticas de las tablas el plan se degrada a 46 s** (hallazgo en `ESTADO.md`): en producción las mantiene autovacuum |
...
| RNF-27 Límite de tasa | pendiente | 5 | Upstash Redis (sesión 021) |
...
Las reglas están documentadas (RN-34 desde la sesión 019); estas son las que todavía no se hacen cumplir donde deben:
