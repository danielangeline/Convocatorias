...
| RF-88 | El registro, por cualquiera de las dos puertas, exige una **casilla de autorización del tratamiento de datos personales** (Ley 1581 de 2012), sin marcar por defecto. La fecha de aceptación se guarda en el perfil. El consultor sin ella —cuenta anterior a la casilla— la acepta en el editor antes de guardar su perfil, y el servidor rechaza guardar sin ella *(nuevo v6, sesión 021)* | CU-14, 16 | Must |
| RF-84 | La entrada —landing, registro e inicio de sesión— presenta **dos puertas: "Soy empresa o entidad" y "Soy consultor"**. En el registro la puerta fija el rol. En el inicio de sesión solo orienta: la cuenta entra siempre al portal de su rol y, si la puerta no coincide, se le informa y se le lleva al correcto. Una cuenta de administrador no recibe ese aviso y pasa directo a la verificación en dos pasos *(nuevo v6)*. **Con una sesión abierta, la landing no ofrece "Iniciar sesión" ni "Crear cuenta" en su cabecera, sino "Ir a mi portal", que lleva al portal del rol por el mismo camino que el inicio de sesión** *(mod. v6, sesión 016)* | CU-14 | Must |
...
| RF-05 | Crear convocatoria con nombre, entidad, descripción, monto, **cobertura (nacional o uno o más departamentos, RN-34; con detalle opcional en texto)** *(mod. v6, sesión 019)*, fechas **y el enlace oficial de postulación (URL del portal de la entidad)** *(mod. v5)*. **Los montos se capturan en pesos enteros y en formato colombiano, con puntos de miles; un formato ambiguo se rechaza con mensaje (CU-02 2b)** *(mod. v6, sesión 016)* | CU-02 | Must |
...
| RF-12 | Filtros combinables: tipo de proyecto, sector, entidad, monto, **departamento (incluye siempre las de cobertura nacional)** *(mod. v6, sesión 019)*, cierre | CU-07 | Must |
| RF-13 | Ficha de detalle con datos, requisitos y descarga de documentos | CU-08 | Must |
| RF-14 | Registrar proyectos con sus datos de clasificación: nombre, monto buscado, **departamento (uno, RN-34) y detalle opcional de ubicación** *(mod. v6, sesión 019)* y categorías | CU-09 | Must |
...
| RF-16 | Mostrar cada sugerencia con **porcentaje de compatibilidad** y desglose de criterios que coinciden y que no *(mod. v4)*. **Cinco criterios, siempre los mismos: tipo de proyecto, sector, tipo de entidad, monto y ubicación; el porcentaje es coincidentes / 5, redondeado. Se calcula en el servidor (docs/05 §9.6). Sin ninguna coincidencia, se indican los datos que le faltan al proyecto y las vigentes más cercanas (CU-10 2a)** *(mod. v6, sesión 019)* | CU-10 | Must |
| RF-43 | **Chips de búsqueda sugerida** clickeables en el buscador y en el estado sin resultados, que precargan combinaciones de filtros *(nuevo v4)*. **Se calculan a partir de las convocatorias vigentes, así que cada chip devuelve al menos un resultado** *(mod. v6, sesión 016)* | CU-07 | Should |
...
| ID | Requerimiento | CU | Prioridad |
|---|---|---|---|
| RF-26 | Directorio solo con consultores aprobados, activos y con suscripción vigente, con filtros por especialidad y rating. **Los consultores del equipo interno no aparecen: llegan solo por asignación del administrador (CU-26)** *(mod. v6, sesión 022, decisión del Product Owner)* | CU-20 | Must |
...
| RF-80 | Revelar sitio web, redes sociales y hoja de vida de un consultor únicamente cuando exista una solicitud activa **entre ese consultor y la empresa autenticada**; la existencia de solicitudes de otras empresas no habilita la visibilidad. **Una solicitud es activa solo cuando el consultor la aceptó: mientras su encargo está `en_curso`**. Una solicitud `pendiente` no revela nada; al completarse, calificarse, rechazarse o cancelarse, los datos vuelven a ocultarse (RN-12, RNF-16) *(precisado en v6, sesión 003; **mod. v6, sesión 022, decisión del Product Owner: antes también revelaba la `pendiente`**)* | CU-21 | Must |
