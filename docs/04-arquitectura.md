...
| Búsqueda y Compatibilidad | `GET /api/convocatorias?q=&tipoProyecto=&sector=&entidad=&ubicacion=&montoHasta=&cierraAntesDe=` (texto libre y filtros combinables de RF-12; las categorías van como ids separados por comas; `montoHasta` en pesos, en formato colombiano) · `GET /api/proyectos/[id]/sugerencias` (devuelve % y desglose) · `GET /api/indicadores` (landing: las cuatro cifras de `indicadores_catalogo()`, con caché de 1 hora; **pública**, porque son agregados que no exponen ninguna convocatoria — RN-33) | RN-05, RF-16, RF-44 |
