...
| `/consultores` · `/consultores/[id]` | Directorio y perfil — **redes/web/CV solo con solicitud aceptada (encargo `en_curso`) de la empresa de la sesión (RF-80, mod. sesión 022)**; sin el equipo interno (RF-26); **botón "Solicitar a este consultor" con selector de proyecto o postulación (RF-74)** | empresa | CU-20, 21 |
...
| Búsqueda y Compatibilidad | `GET /api/convocatorias?q=&tipoProyecto=&sector=&entidad=&departamento=&montoHasta=&cierraAntesDe=&incluirCerradas=` (texto libre y filtros combinables de RF-12; **las cerradas solo vienen con `incluirCerradas=true`**, después de las vigentes y marcadas como `cerrada` —RF-11, RN-02, *sesión 017*—; las categorías van como ids separados por comas; `montoHasta` en pesos, en formato colombiano; `departamento` es el código DANE y trae también las de cobertura nacional —RN-34, *sesión 019*—) · `GET /api/proyectos/[id]/sugerencias` (devuelve % y desglose; *sesión 019:* calcula `sugerencias_proyecto()` en la base con la sesión de la empresa —un proyecto ajeno responde 404—, **402 sin suscripción vigente** (CU-10 2b), y sin coincidencias devuelve los datos que le faltan al proyecto y hasta 5 vigentes cercanas en 0 % (CU-10 2a); solo rol `empresa`) · `GET /api/indicadores` (landing: las cuatro cifras de `indicadores_catalogo()`, con caché de 1 hora; **pública**, porque son agregados que no exponen ninguna convocatoria — RN-33) | RN-05, RF-16, RF-44 |
| Proyectos | `GET/POST /api/proyectos` · `GET/PATCH/DELETE /api/proyectos/[id]` (incluye campos de contenido y completitud). *Sesión 018:* leen y escriben con la sesión de la empresa —**la RLS decide qué existe**: un proyecto ajeno responde 404— y guardan con `guardar_proyecto` (docs/05 §9.17); montos en formato colombiano; `completitud` la calcula la base; borrar responde 409 si el proyecto tiene encargos. Solo rol `empresa` (RNF-30) | RLS por usuario, RN-30 |
| Postulaciones | *(Sesión 020, Sprint 3 pasos 3 y 4 — docs/05 §9.19. Todos con la sesión de la empresa, declarados solo para `empresa` en `lib/autorizacion/matriz.ts` y con segunda barrera: 401 sin sesión, 403 con otro rol o con otro origen en escrituras.)* `GET /api/postulaciones` (las de la sesión, con su convocatoria aunque ya esté cerrada) · `POST /api/postulaciones` con `iniciar_postulacion()` (**verifica estado y vigencia de la convocatoria en el servidor, RF-78**: la garantía es el trigger `privado.exigir_convocatoria_vigente()` —docs/05 §9.7, *sesión 017*—, y el endpoint responde 409 a su clave `convocatoria_no_vigente`; 402 sin suscripción; 404 si la convocatoria o el proyecto no existen para la sesión; **201 si la crea y 200 con la que ya estaba en curso para ese par proyecto-convocatoria, RN-35**) · `GET /api/postulaciones/[id]` (detalle con checklist e historial; ajena → 404) · `PATCH /api/postulaciones/[id]` (vincular el proyecto a la que no tiene, CU-13 3a; ya vinculado o proyecto con otra en curso en esa convocatoria → 409) · `PATCH /api/checklist/[itemId]` (`{ completado }`; en una cerrada → 409, RN-35) · `POST /api/postulaciones/[id]/estado` (`{ estado, confirmado }`; fuera del grafo → 409, RF-83; cerrar sin `confirmado: true` → 400) | RN-03, RN-04, **RN-35**, RF-83 |
| Consultores y Perfiles | `GET/PATCH /api/consultor/perfil` (datos, especialidades, redes y portafolio en una transacción; sin consentimiento, 409, RF-88) · `POST /api/consultor/perfil/consentimiento` · `POST /api/consultor/perfil/archivos/subida` (URL firmada para la foto o la hoja de vida, con la ruta que decide el servidor) · `POST /api/consultor/perfil/archivos` (registra el archivo subido, con tipo y tamaño leídos de Storage) · `GET /api/consultor/perfil/archivos/[tipo]` (URL firmada de 15 min) · `POST /api/consultor/perfil/enviar-revision` (mínimos verificados en la base, CU-17) *(detallado en la sesión 021)* · **Panel** *(sesión 022, Sprint 4 paso 1b — docs/05 §9.21; cuelgan de `/api/admin`, así que heredan el 404 y el MFA del panel)*: `GET /api/admin/consultores?estado=` (todos, o la bandeja con `estado=en_revision`) · `POST /api/admin/consultores/[id]/aprobar` · `POST .../rechazar` (`{ motivo }`, obligatorio, RN-13) · `POST .../suspender` (`{ motivo }`, obligatorio; cancela sus encargos `en_curso` y `pendiente`, RN-29) · `POST .../reactivar` · `GET .../cv` (URL firmada de 15 min) — si el perfil ya no está en el estado que la acción exige, 409 · **Directorio** *(sesión 022, Sprint 4 paso 1c — docs/05 §9.22; solo `empresa`)*: `GET /api/consultores` (aprobados, sin el equipo interno; filtros `q`, `especialidad` —ids separados por comas— y `ratingMin`) · `GET /api/consultores/[id]` (perfil público con reseñas; **web, redes y `tieneHojaDeVida` solo con encargo `en_curso` de la empresa autenticada — RF-80**; 404 si no está en el directorio) · `GET /api/consultores/[id]/cv` (URL firmada de 15 min; 404 sin encargo `en_curso`) | RN-08, RN-12, RNF-16 |
...
| **Generación Documental IA** | `POST /api/documentos/generar` (proyectoId + convocatoriaId — **verifica en el servidor que la convocatoria siga `publicada` y vigente antes de actuar, RF-78**; además, el trigger de `documentos_generados` lo garantiza aunque se escriba con `service_role` —*sesión 017*—, y **la vigencia se comprueba antes de llamar a la IA**, para no gastar una generación que luego no se podría guardar) · `GET/PATCH /api/documentos/[id]` · `POST /api/documentos/[id]/ajustar` (**valida el cupo de la empresa dueña antes de aplicar, también cuando lo pide el consultor — RF-77; sin cupo, 402/403 sin modificar el documento**) · `POST /api/documentos/[id]/compartir` / `.../revocar` (autoriza o quita al consultor — solo el dueño, solo con encargo `en_curso`, RF-71). **El acceso se evalúa como (autorización ∧ encargo `en_curso`) en cada petición, no como permiso almacenado (RN-27), y se revoca en cascada al cerrarse el encargo, suspenderse el perfil o vencer la suscripción (RF-76)** · `GET /api/documentos/[id]/exportar` (devuelve .docx — **403 si quien llama no es el dueño**, RF-72) · `GET/POST /api/admin/plantillas` | RN-17, RN-19..22, 27, 28, RNF-23 |
