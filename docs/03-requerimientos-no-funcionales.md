...
| RNF-25 | **Cobertura de RLS** | Row Level Security habilitado y con política explícita en las 29 tablas sin excepción (27 hasta la sesión 018); ninguna tabla nueva se despliega sin su política escrita y probada *(nuevo v5)* | Revisión de esquema: cada tabla tiene ≥1 política activa; lectura cruzada por tabla entre 2 cuentas → denegada |
...
| ID | Atributo | Requerimiento | Verificación |
|---|---|---|---|
| RNF-16 | Protección de datos personales | Datos del consultor bajo consentimiento explícito (Ley 1581 de 2012). Hoja de vida solo por URLs firmadas con expiración **máxima de 15 minutos** *(precisado en v5)*. Derecho a eliminación del perfil. **La visibilidad de hoja de vida, sitio web y redes es por pareja empresa-consultor, nunca global (RN-12, RF-80)** *(precisado en v6)*. **Los tres buckets son privados y todo archivo se entrega por URL firmada de máximo 15 minutos, no solo la hoja de vida** *(precisado en v6, sesión 012: un bucket público sirve el archivo a quien adivine su ruta, y los adjuntos de convocatoria solo pueden verlos las cuentas de empresa — RN-33)* | Acceso directo a la URL del CV sin autorización → denegado; URL reutilizada pasados 15 min → denegada. **Ruta de un adjunto de convocatoria pedida sin firmar → denegada.** **Prueba cruzada: dos empresas sobre el mismo consultor, una con solicitud activa y otra sin ella → solo la primera ve contacto, CV y redes** |
| RNF-18 | Archivos de perfil **y adjuntos de convocatoria** | Foto ≤5 MB (JPG/PNG); hoja de vida ≤10 MB (PDF). **Adjuntos de convocatoria (RF-07) ≤20 MB, en PDF, Word (.doc/.docx), Excel (.xls/.xlsx) o ZIP** *(precisado en v6, sesión 012: es lo que publican las entidades —términos en PDF, formatos editables, anexos empaquetados— y 20 MB absorbe un TDR escaneado)*. Validación en cliente y servidor; **el límite de tamaño y la lista de tipos se declaran además en el propio bucket de Storage, de modo que una subida directa que los incumpla se rechaza aunque nadie la revise antes** | Archivos límite y tipos inválidos, **también subiendo directo al bucket sin pasar por la pantalla** |
...
| ID | Regla |
|---|---|
| RN-01 | Una convocatoria es visible solo cuando el admin completa **la ficha entera** y publica. Publicar exige: nombre, entidad convocante, **cobertura: nacional o al menos un departamento (RN-34)** *(mod. v6, sesión 019)*, **descripción u objeto**, fecha de cierre no vencida, **enlace oficial de postulación**, **al menos una categoría**, **al menos un documento adjunto** y **al menos dos requisitos** *(ampliado en v5; lista fijada en v6, sesión 015, por decisión del Product Owner tras recorrer el panel)*. **Historia de los adjuntos:** v5 los exigía, la sesión 012 los hizo opcionales —por si una entidad anuncia antes de colgar los términos— y la 015 los vuelve a exigir: una ficha sin documentos deja a la empresa sin qué leer y a la generación con IA sin el texto del TDR. Cobertura, categorías y montos son además los filtros del catálogo (RF-12): sin ellos la convocatoria no aparece cuando alguien filtra. **Una publicada no puede quedar incompleta:** editarla o quitarle adjuntos se rechaza si deja de cumplir la lista; para quitar algo, primero se despublica *(sesión 016: comprobación completa al guardar restaurada y último adjunto protegido)* |
| RN-02 | Convocatoria vencida pasa automáticamente a "cerrada". **Vence al terminar su día de cierre en hora de Colombia** *(precisado en v6, sesión 017)*. Queda **excluida de las sugerencias y de la generación con IA sin excepción**, y **fuera del listado del catálogo por defecto**; solo reaparece en el catálogo si el usuario activa el filtro explícito de convocatorias cerradas, donde se muestra marcada como tal y con sus acciones deshabilitadas (RF-11, RN-03) *(precisado en v6)* |
...
| RN-12 | La hoja de vida, **el sitio web y las redes sociales** solo son visibles para administradores y para **la empresa que tiene la solicitud activa con ese consultor** —**activa = aceptada por el consultor, encargo `en_curso`** *(mod. v6, sesión 022)*— — la visibilidad es por pareja empresa-consultor, nunca global: que otra empresa tenga una solicitud abierta no habilita a las demás. Sin solicitud propia aceptada, la empresa solo ve foto, nombre, descripción, especialidades, portafolio (sin links de contacto), rating y reseñas *(ampliado en v5; precisado en v6)* |
...
| RN-29 | Suspender un consultor (CU-27) cancela de inmediato sus encargos `en_curso` **y sus solicitudes `pendiente`** *(mod. v6, sesión 022)* — pasan a `cancelado` con un motivo registrado — sin alterar el historial ni las calificaciones ya emitidas; reactivar el perfil no los revive. El vencimiento de la suscripción del consultor (RN-10, CU-18) produce el mismo efecto sobre el dato, con un disparador distinto (el job diario, CU-32, en vez de una acción del administrador). **Ambos disparadores arrastran también la revocación de las autorizaciones de documento (RF-76)** *(nuevo v5; ampliado en v6)* |
...
| RN-34 | **La ubicación se expresa con la lista oficial de departamentos de Colombia** (los 32 departamentos y Bogotá D.C., con su código DANE), no con texto libre: una convocatoria es de **cobertura nacional** o cubre **uno o más departamentos**, y un proyecto se ejecuta en **un departamento**. El texto libre se conserva solo como detalle (municipios, zona) para leer, nunca para comparar. Así el filtro del catálogo (RF-12) y la compatibilidad (RF-16) no dependen de cómo alguien escribió un nombre *(nuevo v6, sesión 019, por decisión del Product Owner)* |
| RN-35 | **Una postulación en curso por par proyecto-convocatoria.** Una empresa tiene como máximo una postulación no cerrada por cada par proyecto-convocatoria (las que no tienen proyecto cuentan como un par propio); presentar dos proyectos distintos a la misma convocatoria sí se admite. **El proyecto se vincula una sola vez**: la postulación sin proyecto puede recibir uno, pero una vez vinculado no se cambia, porque el documento generado y los encargos cuelgan de ese par. **El checklist se marca hasta que la postulación se cierra**; cerrada, queda de solo lectura. Las tres cosas las hace cumplir la base, no la pantalla *(nuevo v6, sesión 020, por decisión del Product Owner)* |
